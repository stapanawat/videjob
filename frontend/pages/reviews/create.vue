<template>
  <div class="min-h-screen pt-24 pb-12 flex items-center justify-center px-6">
    <div class="w-full max-w-lg bg-white dark:bg-surface-800 rounded-2xl shadow-xl border border-surface-200 dark:border-surface-700 p-8">
      ...
      <div v-if="!submitted">
        <h1 class="text-2xl font-bold text-surface-900 dark:text-white mb-2 text-center">ให้คะแนนการทำงาน</h1>
        <p class="text-center text-surface-500 dark:text-surface-400 mb-8">งาน: Barista @ Aroma Coffee (20 Feb)</p>
        ...
        <form @submit.prevent="submitReview" class="space-y-6">
          ...
          <div class="space-y-4 bg-surface-50 dark:bg-surface-900 p-4 rounded-xl border border-surface-100 dark:border-surface-700">
             <div class="flex flex-wrap items-center justify-between gap-2">
               <span class="text-sm font-medium text-surface-700 dark:text-surface-300">ความตรงต่อเวลา</span>
             ...
             <div class="flex flex-wrap items-center justify-between gap-2">
               <span class="text-sm font-medium text-surface-700 dark:text-surface-300">ความตั้งใจทำงาน</span>
               <div class="flex gap-1 text-surface-300">
          ...
          <div class="space-y-2">
            <label class="text-sm font-medium text-surface-700 dark:text-surface-300">ความคิดเห็นเพิ่มเติม</label>
            <textarea v-model="comment" rows="4" placeholder="เล่าประสบการณ์การทำงาน..." class="w-full bg-surface-50 dark:bg-surface-900 border-surface-200 dark:border-surface-700 rounded-xl px-4 py-3 text-surface-900 dark:text-white focus:border-primary-500 focus:outline-none placeholder:text-surface-400"></textarea>
          </div>

          <div class="flex items-center gap-3">
             <input type="checkbox" id="anon" class="w-4 h-4 rounded border-surface-300 text-primary-600 focus:ring-primary-500">
             <label for="anon" class="text-sm text-surface-600 dark:text-surface-400">รีวิวโดยไม่เปิดเผยชื่อ</label>
          </div>
      ...
      <div v-else class="text-center py-8">
        ...
        <h2 class="text-2xl font-bold text-surface-900 dark:text-white mb-2">ขอบคุณสำหรับการรีวิว!</h2>
        <p class="text-surface-500 dark:text-surface-400 mb-8">ความคิดเห็นของคุณช่วยสร้างสังคมการทำงานที่ดีขึ้น</p>
        <NuxtLink to="/" class="inline-block px-8 py-3 bg-surface-100 dark:bg-surface-700 hover:bg-surface-200 dark:hover:bg-surface-600 text-surface-900 dark:text-white font-medium rounded-xl transition-colors">
          กลับหน้าหลัก
        </NuxtLink>
      </div>
